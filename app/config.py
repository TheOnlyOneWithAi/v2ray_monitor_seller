...
class Settings(BaseSettings):
    ...
    @property
    def admins(self): return {int(x.strip()) for x in self.admin_ids.split(',') if x.strip().isdigit()}
    @property
    def key(self): return self.encryption_key.encode() if self.encryption_key else Fernet.generate_key()
settings=Settings()

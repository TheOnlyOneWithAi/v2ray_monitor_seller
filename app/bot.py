...
async def start(m:Message,bot:Bot):
    ...
    await ensure_user(m.from_user);ps=await plans();welcome=await cfg('welcome','🛍 فروشگاه کانفیگ\nیک پلن انتخاب کنید:');await m.answer(welcome,reply_markup=plan_buttons(ps))
...
async def approve(c:CallbackQuery,bot:Bot):
    ...
    async with Session() as s:
        ...
        p=await s.get(Plan,o.plan_id);u=await s.get(User,o.user_id);o.status='paid';o.paid_at=datetime.now(timezone.utc);s.add(Subscription(user_id=o.user_id,plan_id=p.id,config_count=0,expires_at=datetime.now(timezone.utc)+timedelta(days=p.days)));await s.commit()
    await bot.send_message(u.telegram_id,f'✅ پرداخت سفارش #{oid} تأیید شد. اشتراک {p.name} فعال شد و تا {p.days} روز اعتبار دارد.\nظرفیت: {p.max_configs} کانفیگ.');await c.message.edit_text(f'سفارش #{oid} تأیید شد.');await c.answer()
